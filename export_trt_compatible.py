...
def export_trt_compatible_models():
    """Export TensorRT-compatible SuperPoint extractor and LightGlue matcher"""
    
    print("🚀 Exporting TensorRT-compatible models...")
    ...
    extractor_output = weights_dir / "superpoint_extractor_trt.onnx"
    matcher_output = weights_dir / "superpoint_lightglue_matcher_trt.onnx"
    ...
    dummy_input = {"image": torch.zeros(batch_size, 1, height, width)}
    ...
    onnx.save_model(
        SymbolicShapeInference.infer_shapes(onnx.load_model(str(matcher_output)), auto_merge=True),
        str(matcher_output)
    )
