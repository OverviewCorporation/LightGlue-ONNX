...
class TRTCompatibleLayerNorm(nn.Module):
    """LayerNorm implementation that's more compatible with TensorRT"""
    ...
    def forward(self, x):
        # Ensure operations use float32 for better TensorRT compatibility
        input_dtype = x.dtype
        x = x.float()
...
class SelfBlock(nn.Module):
    def __init__(self, embed_dim: int, num_heads: int, bias: bool = True) -> None:
        ...
        self.ffn = nn.Sequential(
            ...
            TRTCompatibleLayerNorm(2 * embed_dim, elementwise_affine=True),
            nn.GELU(),
...
class LightGlueTRT(nn.Module):
    def __init__(
        ...
        # Load state dict with strict=False to handle LayerNorm parameter changes
